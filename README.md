...
```sh
go install github.com/waozixyz/pass/cmd/pass@latest
```
...
XWayland) is reachable, so no external tool is needed; otherwise `pass`
...
An optional configuration file at `$XDG_CONFIG_HOME/pass/profiles.json`
(usually `~/.config/pass/profiles.json`) can store named profiles — a login
plus default settings — so daily invocations shrink to one flag:

```sh
pass --profile work example.com
```
...
and without a configuration file pass behaves exactly as before.
...
pass --vault ~/.vault example.com alice
```
...
```go
settings := pass.DefaultOptions()
result, err := pass.Generate("example.com", "alice", master, settings)
```
