...
cp "$root/packaging/linux/xyz.waozi.gopass.desktop" "$appdir/usr/share/applications/xyz.waozi.gopass.desktop"
cp "$root/packaging/linux/xyz.waozi.gopass.appdata.xml" "$appdir/usr/share/metainfo/xyz.waozi.gopass.appdata.xml"
cp "$root/assets/app/icon.png" "$appdir/usr/share/icons/hicolor/512x512/apps/gopass.png"
rm -f "$root"/gopass-*.AppImage
ARCH="$arch" "$linuxdeploy" --appdir "$appdir" --desktop-file "$root/packaging/linux/xyz.waozi.gopass.desktop" --icon-file "$root/assets/app/icon.png" --output appimage
